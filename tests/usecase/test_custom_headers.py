...
class TestCaseCustomHeaderConfig(unittest.TestCase):
    ...
    def test_valid_header_referrerPolicy(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"Referrer-Policy": "no-referrer-when-downgrade"}'
        header_config = start.parse_header()
        assert "add_header Referrer-Policy 'no-referrer-when-downgrade';" in header_config

    def test_invalid_header_referrerPolicy(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"Referrer-Policy": "no-referrr-when-downgrade"}'
        header_config = start.parse_header()
        assert "" in header_config

    def test_valid_header_accessControl(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"Access-Control-Allow-Origin": "*"}'
        header_config = start.parse_header()
        assert "add_header Access-Control-Allow-Origin '*';" in header_config

    def test_invalid_header_accessControl(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"Access-Control-Allow-Origin": "htps://this.is.mydomain.nl"}'
        header_config = start.parse_header()
        assert "" in header_config

    def test_valid_header_contentType(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"X-Content-Type-Options": "nosniff"}'
        header_config = start.parse_header()
    ...
    def test_valid_header_permittedPolicies(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"X-Permitted-Cross-Domain-Policies": "by-content-type"}'
        header_config = start.parse_header()
        assert "add_header X-Permitted-Cross-Domain-Policies 'by-content-type';" in header_config

    def test_invalid_header_permittedPolicies(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"X-Permitted-Cross-Domain-Policies": "#%#^#^"}'
        header_config = start.parse_header()
        assert "" in header_config

    def test_valid_header_xssProtection(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"X-XSS-Protection": "1; report=https://domainwithnewstyle.tld.consultancy"}'   # noqa: E501
        header_config = start.parse_header()
        assert "add_header X-XSS-Protection '1; report=https://domainwithnewstyle.tld.consultancy';" in header_config   # noqa: E501

    def test_invalid_header_xssProtection(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"X-XSS-Protection": "1;mode=bock"}'
        header_config = start.parse_header()
        assert "" in header_config

    def test_valid_header_partial(self):
        os.environ["HTTP_RESPONSE_HEADERS"] = '{"Referrer-Policy": "no-referrr-when-downgrade","Access-Control-Allow-Origin": "https://this.is.mydomain.nl","X-Content-Type-Options": "nosniff"}'   # noqa: E501
        header_config = start.parse_header()
        assert "add_header X-XSS-Protection '1; report=https://domainwithnewstyle.tld.consultancy';" not in header_config   # noqa: E501
